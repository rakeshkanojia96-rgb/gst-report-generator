...
class GSTReportGenerator {
    ...
    async generateReport() {
        ...
        if (this.amazonFiles.length === 0 && this.meeshoFiles.length === 0) {
            alert('🔥 ERROR: No files selected! Please upload Amazon or Meesho files first.');
            return;
        }
    ...
    processMeeshoHSNData() {
        ...
        if (this.processedData.meesho.tcs_sales) {
            ...
            console.log(`🔥 HSN Total Quantity set to TCS Sales record count: ${tcsRecordCount}`);
            
            Object.keys(hsnData).forEach(hsn => {
                hsnData[hsn].quantity = tcsRecordCount;
            });
            
            console.log('🔥 HSN QUANTITY SET TO:', this.processedData.meesho.tcs_sales.length);
        ...
        Object.entries(hsnData).forEach(([hsn, data]) => {
            ...
            console.log(`🔥 Meesho HSN ${hsn}: Final Qty=${finalQuantity} (TCS Sales count), Value=${data.taxableValue}`);
            meeshoHSN.push({
                'HSN Code': hsn,
                'Description': 'OF COTTON',
                'UQC': 'PCS',
